...
struct {
  ...
  void open() {
    ...
    DISP.showStatusChar("O");
  }

  void close() {
    status = CLOSED;
    digitalWrite(SQUELCH_OUT, HIGH);
    DISP.showStatusChar("C");
  }
...
struct {
 ...
 // boolean lockout[nChannels] = {1,0,0,0,0};
  boolean lockout[nChannels] = {0,1,1,1,1};
  int channel;
  ...
  void spinnerNext() {
    const char spinner[] = {'|', '/', '-', '\\'};
    DISP.showStatusChar(spinner[channel++ % 4]);
    }
...
public:
  ...
  regularTask(unsigned int interval, void (*cb)()) {
